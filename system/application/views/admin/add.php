...
<html>
	<head>
		...
		<title>the Splash : Add Project</title>
		...
		<section id="container">
			<header>
				<ul id="navigation">
					...
					<li id="add_project" class="nav_item selected"><a href="<?=site_url('admin')?>" class="selected">Add Project</a></li>
				...
				<?php foreach ($fields->result() as $field):?>
					<tr>
						...
						if ($field->fieldType == 'text') {
							?><td class="field"><input type="text" name="field_<?=$field->fieldID?>" id="field_<?=$field->fieldID?>" /></td><?
						}
						elseif ($field->fieldType == 'textarea') {
							?><td class="field"><textarea name="field_<?=$field->fieldID?>" id="field_<?=$field->fieldID?>"></textarea></td><?
						}
									...
									<?php if (is_array($file)):?>
										<optgroup label="<?=$key?>">
											<?php for ($i=0; $i<count($file); $i++):?>
												<option value="<?=$key?>/<?=$file[$i]?>"><?=$file[$i]?></option>
											<?php endfor; ?>
										</optgroup>
									<?php else: ?>
